...
class ClassName(BaseScript):  # Название класса (должен отличаться от других названий скриптов)
    ...
    def __init__(self):
        ...
        self.model = YOLO('best6small50.pt')  # load a pretrained YOLOv8n model
    ...
    def MouseMove(self, box, img_w=640, img_h=640, scale=1, currentMousemove=None, limit=400):
        ...
        x = int(10 * r1 / 1.125 * 90 / 60)  #2.8 UE 5
    ...
    def fastselfcast(self, spell, casttime):
        ...
        if self.blackscreen:
            ...
            self.checker.join()
            if not self.ban and not self.mainmenu:
                self.send_message_telega(f"BANISHED in fastselfcast")
            sys.exit()
        if self.lowmana:
            print("LOWMANA")
            self.restart = True
