...
class ClassName(BaseScript):  # Название класса (должен отличаться от других названий скриптов)
    ...
    def mousemove(self, x, y, timer=0.007, limiter=120):
        ...
        self.aftermovedelay= 0.025+0.015*(abs(x)+abs(y))/120
        if self.aftermovedelay > 0.06:
    ...
    def track(self, checkbox, precision=0.6, conf=0.05, i=1):
        ...
        for j in range(i):
            ...
            if found:
                if time()-self.holdtime > 0.21 and newboxdist*2.2 > (newbox.xyxy[0][2] - newbox.xyxy[0][0]):
                    return False, None
                return True, newbox
