...
class ClassName(BaseScript):  # Название класса (должен отличаться от других названий скриптов)
            ...
            if len(detected_boxes) >= 1:
                ...
                if not results is None:
                    ...
                    if self.spiritdetect():
                        ...
                        self.lkmpress()
                        spamtimer = time()
                        while self.spiritdetect() and time() - nospirittime < 17.5:
                            self.getNextFrame()
                            Prediction = self.model.predict(source=self.img, device=0, conf=0.12, iou=0.2)
                            # print(Prediction[0].boxes.xyxy)
    ...
    def startLoop(self):
        ...
        while (start_loop):
            ...
            if self.ultrasave and released and (time() - nospirittime > 2.5):
                self.lkmrelease()
    ...
    def delete_cuprum2(self):
        ...
        # Read the images from the file
        img = self.img
        mxLoc = self.imgfind(img, "cuprum2.png", loc=True, conf=0.95)
    ...
    def delete_cuprum1(self):
        ...
        # Read the images from the file
        img = self.img
        mxLoc = self.imgfind(img, "cuprum1.png", loc=True, conf=0.95)
        if mxLoc is not None:
            self.deleteLoc(mxLoc)
    ...
    def delete_cuprum(self):
        ...
        # Read the images from the file
        img = self.img
        mxLoc = self.imgfind(img, "cuprum.png",loc=True,conf=0.95)
        if  mxLoc is not None:
            self.deleteLoc(mxLoc)
    ...
    def custom(self):
        ...
        while True:
            ...
            if self.mover is None:
                self.mover = Thread(target=self.hold_and_release_wait, args=(self.moveback, self.MaxMoveBackTimer))
                self.mover.start()
            else:
                self.mover.join()
                self.mover = Thread(target=self.hold_and_release_wait, args=(self.moveback, self.MaxMoveBackTimer))
            ...
            if self.mover is None:
                self.mover = Thread(target=self.hold_and_release_wait, args=(self.moveforward, self.MaxMoveBackTimer*self.MoveForwardMultiplier))
                self.mover.start()
            else:
                self.mover.join()
                self.mover = Thread(target=self.hold_and_release_wait, args=(self.moveforward, self.MaxMoveBackTimer*self.MoveForwardMultiplier))
